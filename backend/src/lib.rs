//! # A Love Story - Cloudflare Worker (Rust)
//!
//! Ed25519 signature verification - Optimized stateless version
//! 
//! This is the high-performance Rust WASM version.
//! For D1 database integration, use the JavaScript Worker (backend-js/).
...
const WORKER_VERSION: &str = "rust-1.0.0";
...
async fn main(req: Request, _env: Env, _ctx: Context) -> Result<Response> {
    ...
    Router::new()
        ...
        .post_async("/register", |mut req, _| async move {
            ...
            match VerifyingKey::from_bytes(&reg.public_key) {
                Ok(_) => {
                    let resp = RegisterResponse {
                        user_id: get_user_id(&reg.public_key),
                        short_user_id: get_short_user_id(&reg.public_key),
                        success: true,
                    };
